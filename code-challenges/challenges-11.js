...
function fullName(arr) {
    ...
    const fName = arr.map(x => `${x.firstName} ${x.lastName}`)
...
function gradesAvg(arr) {
    ...
    let avgArray = arr.map((grads => ({
        ...grads,


        avg: (grads.gradsList.reduce((x, y) => x + y)) / grads.gradsList.length
    })))
